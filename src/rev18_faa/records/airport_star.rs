    ...
    pub fn parse(input: &[u8]) -> Result<ARINCRecord<'_>, RecordParseError> {
        if is_primary_record(input, Self::CONTINUATION_COLUMN) {
            ...
            if let Ok(Some(application_type)) = ContinuationRecordApplicationType::from_bytes(
                ...
                Ok(ARINCRecord::AirportSTARChangedDataContinuation(
                    AirportSTARChangedDataContinuationRecord::parse(input)?,
                ))
            }
        }
    }
...
impl<'a> Arinc424RecordSpec<'a> for AirportSTARPrimaryRecord<'a> {
    ...
    fn group_key(&self) -> GroupKey {
        GroupKey::from_byte_slices(&[
            ...
            self.star_identifier.raw_bytes,
        ])
    }
}
...
pub type AirportSTARChangedDataContinuationRecord<'a> = AirportSTARPrimaryRecord<'a>;
