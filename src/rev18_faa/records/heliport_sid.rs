    ...
    pub fn parse(input: &[u8]) -> Result<ARINCRecord<'_>, RecordParseError> {
        if is_primary_record(input, Self::CONTINUATION_COLUMN) {
            ...
            if let Ok(Some(application_type)) = ContinuationRecordApplicationType::from_bytes(
                ...
                Ok(ARINCRecord::HeliportSIDChangedDataContinuation(
                    HeliportSIDChangedDataContinuationRecord::parse(input)?,
                ))
            }
        }
    }
...
pub type HeliportSIDChangedDataContinuationRecord<'a> = HeliportSIDPrimaryRecord<'a>;
